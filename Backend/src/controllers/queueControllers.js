    ...
    takeQueueById: async (req, res) => {
        try {
          ...
          const { rows: queueToUpdate } = await pool.query(
            `SELECT * FROM Queue WHERE process_status = 'IN_QUEUE' ORDER BY queue_id LIMIT 1`
          );
      
          if (queueToUpdate.length === 0) {
            return res.status(404).json({ error: 'No queue entry in queue status found' });
          }
          ...
          await pool.query(
            `UPDATE Queue SET teller_id = $1, process_status = $2 WHERE queue_id = $3`,
            [teller_id, status.processing, queueIdToUpdate]
          );
